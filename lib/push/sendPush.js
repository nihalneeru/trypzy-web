...
const PUSH_COPY = {
  ...
  leader_ready_to_propose: (tripName) => ({
    title: 'Trypzy',
    body: `"${tripName}" has a promising date range. Propose when you're ready.`,
  }),
  dates_locked: (tripName) => ({
    title: 'Trypzy',
    body: `"${tripName}" dates are locked! Time to plan the fun stuff.`,
  }),
}
...
export async function sendPushForNudge(db, nudge, trip) {
  ...
  const bundleId = process.env.APNS_BUNDLE_ID || 'com.trypzy.mobile'
