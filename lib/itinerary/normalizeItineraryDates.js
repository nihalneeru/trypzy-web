...
export function normalizeItineraryDates(itinerary, dateList) {
  ...
  for (let i = 0; i < dateList.length; i++) {
    ...
    // Ensure day-level fields exist
    if (typeof day.areaFocus !== 'string') day.areaFocus = ''
    if (typeof day.groupFit !== 'string') day.groupFit = ''
    ...
    day.blocks = day.blocks.map(block => ({
      ...
      reservation: block.reservation && typeof block.reservation === 'object'
        ? { needed: !!block.reservation.needed, notes: block.reservation.notes || '' }
        : { needed: false, notes: '' }
    }))
