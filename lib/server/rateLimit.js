...
export async function checkRateLimit(identifier, tier) {
  try {
    ...
    if (!limiter) {
      // Redis not configured — gracefully allow
    ...
    // Redis unavailable — gracefully degrade, never block users
    console.warn('[rateLimit] Redis error, allowing request:', error.message)
    return { success: true, remaining: -1, reset: 0 }
  }
}
