...
export function getTierForRoute(route, method) {
  ...
  // Chat polling (GET messages) — exempt from rate limiting to avoid
  // starving other endpoints (polling every 5s = 720 req/h per user)
  if (method === 'GET' && (
    route.match(/^\/trips\/[^/]+\/messages$/) ||
    route.match(/^\/circles\/[^/]+\/messages$/)
  )) {
    return 'none'
  }

  // Chat messages (POST — sending)
