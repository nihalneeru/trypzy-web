...
const UPGRADE_MODEL = 'gpt-4o'

// Export MODEL for llmMeta observability
export { MODEL as LLM_MODEL }

/**
 * Compute complexity score for itinerary generation/revision.
 * Used to decide whether to upgrade to a more capable model.
 * Feature-flagged: ITINERARY_MODEL_UPGRADE=1 (default OFF).
 *
 * @param {Object} params
 * @param {number} params.numberOfDays - Trip duration in days
 * @param {number} params.ideaCount - Number of ideas
 * @param {number} params.constraintCount - Number of constraints
 * @param {boolean} [params.hasAccommodation] - Whether accommodation context is present
 * @param {boolean} [params.hasChatBrief] - Whether chat brief is present
 * @returns {{ score: number, model: string, factors: Object }}
 */
export function computeComplexityScore({
  ...
  // Days: 0-30 points (1pt per day, capped at 30)
  factors.days = Math.min(30, numberOfDays * (numberOfDays <= 3 ? 5 : numberOfDays <= 5 ? 4 : 3))

  // Ideas: 0-20 points
  factors.ideas = Math.min(20, ideaCount * 2)

  // Constraints: 0-20 points
  factors.constraints = Math.min(20, constraintCount * 4)

  // Accommodation: 0-5 points
  factors.accommodation = hasAccommodation ? 5 : 0

  // Chat brief: 0-10 points
  factors.chatBrief = hasChatBrief ? 10 : 0

  const score = Object.values(factors).reduce((sum, v) => sum + v, 0)
  const upgradeEnabled = process.env.ITINERARY_MODEL_UPGRADE === '1'
  const model = upgradeEnabled && score >= 60 ? UPGRADE_MODEL : MODEL

  return { score, model, factors }
}
 ...
 */
export async function generateItinerary({ destination, startDate, endDate, dateList, groupSize, ideas, constraints = [], chatBrief = null, accommodation = null, occasion = null, durationPreferences = null, tripType = 'collaborative', travelerInterests = null }) {
  if (!OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY environment variable is not set')
  }
  ...
  const systemPrompt = `You are Tripti.ai's itinerary planner for group trips. Output ONLY valid JSON matching the provided schema.

Rules:
- Be specific to the destination, season, and local culture.
- Prefer authentic local favorites over generic tourist options. Include iconic highlights when relevant, scheduled smartly (off-peak, paired with nearby local spots).
- Make realistic day plans: include transit time between locations and buffer time.
- Prioritize the group's signals: top-voted ideas first, must-dos from chat, pace/budget preferences.
- Group activities by neighborhood to minimize backtracking.
- Use "$X-Y per person" or "Free" for estCost — never leave blank.
- If unsure about a detail, note assumptions in overview.notes.`
...
${sections.join('\n')}`
  ...
  // Helper: build enrichment sections (each returns '' if data is null/empty)
  const buildAccommodationSection = () => {
    if (!accommodation) return ''
    const parts = [`Staying at ${accommodation.name}`]
    if (accommodation.location) parts.push(`in ${accommodation.location}`)
    if (accommodation.priceRange) parts.push(`(${accommodation.priceRange})`)
    return `\nACCOMMODATION (daily anchor — plan activities near this area when possible):\n${parts.join(' ')}`
  }

  const buildOccasionSection = () => {
    if (!occasion) return ''
    return `\nTrip occasion: "${occasion}" — weave this theme where natural, but don't force it.`
  }

  const buildDurationSection = () => {
    if (!durationPreferences || !durationPreferences.dominant) return ''
    const label = durationPreferences.dominant
    const respondedInfo = durationPreferences.totalResponses
      ? ` (${durationPreferences.totalResponses} responded)`
      : ''
    const paceHints = {
      weekend: 'Short trip — pack highlights, skip filler.',
      extended: 'Extended weekend — mix highlights with one relaxed half-day.',
      week: 'Week-long — include variety and rest days.',
      week_plus: 'Long trip — build in rest days, deeper exploration, day-trip options.',
      flexible: 'Flexible duration — balanced pacing.'
    }
    return `\nGroup pace preference: ${label}${respondedInfo}. ${paceHints[label] || ''}`
  }

  const buildTripTypeSection = () => {
    if (tripType === 'hosted') {
      return '\nTrip type: Hosted — create a polished, guide-like experience with clear structure.'
    }
    return '\nTrip type: Collaborative — balance different interests, casual tone, leave room for spontaneity.'
  }

  const buildInterestsSection = () => {
    if (!travelerInterests) return ''
    return `\nGroup interests (from ideas): ${travelerInterests}. Lean toward these themes when filling gaps.`
  }
  ...
  const buildPrompt = (ideasToInclude, chatBriefSection = '') => {
    const ideasText = ideasToInclude.length > 0
      ? ideasToInclude.map((idea, idx) => `${idx + 1}. ${idea.title}${idea.details ? ` - ${idea.details}` : ''}${idea.category ? ` [${idea.category}]` : ''}${idea.location ? ` @ ${idea.location}` : ''}${idea.likeCount ? ` [${idea.likeCount} vote${idea.likeCount !== 1 ? 's' : ''}]` : ''}${idea.constraints && idea.constraints.length > 0 ? ` (constraints: ${idea.constraints.join(', ')})` : ''}`).join('\n')
      : 'No specific ideas provided'

    const ideasInstruction = ideasToInclude.some(i => i.likeCount > 0)
      ? '\n(Ideas sorted by group popularity. Prioritize highly-voted ideas.)'
      : ''
...
${ideasText}${ideasInstruction}${constraintsText}${buildAccommodationSection()}${buildOccasionSection()}${buildDurationSection()}${buildTripTypeSection()}${buildInterestsSection()}${chatBriefSection}
...
SCHEDULING RULES:
- 3-5 activity blocks per day (not counting meals or brief transit).
- First day: start no earlier than 14:00 (assume afternoon arrival).
- Last day: end activities by 14:00 (allow departure time).
- Include 30-60 min buffer between activities in different locations.
- Total planned hours per day should not exceed 10.
- Group nearby activities together to minimize transit.

EXAMPLE of one high-quality block (style reference — do NOT copy content):
{
  "timeRange": "10:00-12:00",
  "title": "Explore Pike Place Market",
  "description": "Walk the iconic market — skip front stalls (tourist prices), head to back for fresh juices and local tapas.",
  "location": "Pike Place Market, Seattle",
  "tags": ["food", "sights"],
  "estCost": "$15-25 per person",
  "transitNotes": "10 min walk from hotel",
  "sourceIdeaIds": [],
  "reservation": { "needed": false, "notes": "Arrive before 11am to avoid crowds" }
}

In "planningNotes", list 2-4 assumptions and describe your area strategy. In each day's "groupFit", write ONE sentence explaining why the day suits this group.
  ...
  "days": [
    {
      "date": "YYYY-MM-DD",
      "title": "string",
      "areaFocus": "string (neighborhood/area for this day)",
      "groupFit": "string (one sentence: why this day suits this group)",
      "blocks": [
        {
          ...
          "location": "string",
          "tags": ["food","sights"],
          "estCost": "$X-Y per person or Free",
          "transitNotes": "string",
          "sourceIdeaIds": ["id1", "id2"],
          "reservation": { "needed": true|false, "notes": "string" }
        }
      ]
    }
  ]
...
Important: Output ONLY the JSON object, nothing else.`
  ...
  // IQ-12: Complexity-based model selection
  const complexity = computeComplexityScore({
    numberOfDays,
    ideaCount: ideasToUse.length,
    constraintCount: constraints.length,
    hasAccommodation: !!accommodation,
    hasChatBrief: !!chatBrief
  })
  const selectedModel = complexity.model
  ...
  try {
    const response = await fetchWithRetry(OPENAI_API_URL, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.7,
        max_completion_tokens: Math.min(5000, 1200 + numberOfDays * 550),
        response_format: { type: 'json_object' }
      })
    ...
    const { validateItineraryStructure } = await import('@/lib/itinerary/validateItineraryStructure.js')
    const structResult = validateItineraryStructure(itinerary, dateList)
    if (structResult.repaired) {
      itinerary = structResult.itinerary
    }
    if (structResult.warnings.length > 0 && process.env.NODE_ENV !== 'production') {
      console.warn('[generateItinerary] Structural warnings:', structResult.warnings)
    }
    ...
    return {
      ...
      _meta: {
        ...
        structuralWarnings: structResult.warnings,
        structuralWarningCount: structResult.warnings.length,
        complexityScore: complexity.score,
        modelSelected: selectedModel,
        modelUpgraded: selectedModel !== MODEL
      }
    }
  } catch (error) {
 ...
 */
export async function reviseItinerary({ currentItinerary, feedbackSummary, newIdeas = [], chatMessages = [], destination, startDate, endDate, dateList, accommodation = null, tripType = 'collaborative' }) {
  if (!OPENAI_API_KEY) {
    throw new Error('OPENAI_API_KEY environment variable is not set')
  }
...
- Use "$X-Y per person" or "Free" for estCost — never leave blank.
- If unsure about a detail, note assumptions in overview.notes.
- Keep unchanged items stable unless feedback asks for changes.`
...
Constraints: ${feedbackSummaryToUse.constraints.length > 0 ? feedbackSummaryToUse.constraints.join('; ') : 'none'}`
    ...
    // Build enrichment sections for revision context
    let enrichmentSections = ''
    if (accommodation) {
      const parts = [`Staying at ${accommodation.name}`]
      if (accommodation.location) parts.push(`in ${accommodation.location}`)
      if (accommodation.priceRange) parts.push(`(${accommodation.priceRange})`)
      enrichmentSections += `\nACCOMMODATION (daily anchor): ${parts.join(' ')}`
    }
    if (tripType === 'hosted') {
      enrichmentSections += '\nTrip type: Hosted — maintain polished, guide-like tone.'
    }
...
${feedbackText}${newIdeasText}${enrichmentSections}
...
Output ONLY valid JSON matching the exact same schema as the current itinerary (including planningNotes, areaFocus, groupFit, reservation fields — no markdown, no extra text).
Also provide a brief 1-3 sentence change log describing what was updated.`
  ...
  // IQ-12: Complexity-based model selection for revision
  const complexity = computeComplexityScore({
    numberOfDays,
    ideaCount: newIdeasToUse.length,
    constraintCount: feedbackSummaryToUse.constraints?.length || 0,
    hasAccommodation: !!accommodation,
    hasChatBrief: false
  })
  const selectedModel = complexity.model
  ...
  try {
    ...
    // Structural validation + auto-repair
    const { validateItineraryStructure } = await import('@/lib/itinerary/validateItineraryStructure.js')
    const structResult = validateItineraryStructure(revised, dateList)
    if (structResult.repaired) {
      revised = structResult.itinerary
    }
    if (structResult.warnings.length > 0 && process.env.NODE_ENV !== 'production') {
      console.warn('[reviseItinerary] Structural warnings:', structResult.warnings)
    }
