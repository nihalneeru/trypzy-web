      ...
      if (account?.provider === 'google' && user) {
        try {
          ...
          if (!existingUser) {
            ...
            // Handle users that may have been created by MongoDBAdapter (with _id but not id)
            // or users from our custom collection (with id)
            let userId = existingUser.id
            if (!userId) {
              // User was created by NextAuth adapter, add our custom id field
              userId = uuidv4()
