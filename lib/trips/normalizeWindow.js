...
function parseExplicitDates(text, context) {
  ...
  // Pattern 1a: ISO range "YYYY-MM-DD to YYYY-MM-DD" or "YYYY-MM-DD - YYYY-MM-DD"
  ...
  // Pattern 1b: Single ISO date "YYYY-MM-DD"
  const isoSinglePattern = /^(\d{4})-(\d{2})-(\d{2})$/
  const isoSingleMatch = normalized.match(isoSinglePattern)
  if (isoSingleMatch) {
    const [, y, m, d] = isoSingleMatch
    const iso = `${y}-${m}-${d}`
    return {
      startISO: iso,
      endISO: iso,
...
function parseRelativeMonth(text, context) {
  ...
  // Pattern: "early/mid/late/beginning of/end of Month"
  const relativePattern = /^(early|mid|late|beginning\s+of|end\s+of)\s+([a-z]+)(?:\s*,?\s*(\d{4}))?$/i
  const match = normalized.match(relativePattern)
  if (!match) return null

  const [, rawPosition, monthStr, yearStr] = match
  const position = rawPosition.replace(/\s+/g, ' ')
  const month = parseMonth(monthStr)
  if (month === null) return null
...
/**
 * Parse ordinal week of month patterns
 * Handles: "first week of Feb", "1st week of March", "2nd week of April",
 *          "third week of July", "4th week of December"
 */
function parseOrdinalWeekPattern(text, context) {
  const normalized = text.trim().toLowerCase()

  const pattern = /^(?:the\s+)?(first|1st|second|2nd|third|3rd|fourth|4th)\s+week\s+(?:of\s+)?([a-z]+)(?:\s*,?\s*(\d{4}))?$/i
  const match = normalized.match(pattern)
  if (!match) return null
...
export function normalizeWindow(inputText, context = {}) {
  if (!inputText || typeof inputText !== 'string') {
    return {
      error: 'Please enter a date range. Examples: "Feb 7-9", "early March", "first week of April", "end of June", "April"'
    }
  }
  ...
  if (!result) {
    result = parseOrdinalWeekPattern(trimmed, context)
  }
  if (!result) {
  ...
  if (!result) {
    return {
      error: 'Could not understand the date format. Try: "Feb 7-9", "early March", "first week of April", "last weekend of June", or "end of March"'
    }
  }
