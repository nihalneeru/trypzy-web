...
function parseWeekendPattern(text, context) {
  ...
  // Pattern: "first/1st weekend of Month"
  const weekendPattern = /^(?:first|1st)\s+weekend\s+(?:of\s+)?([a-z]+)(?:\s*,?\s*(\d{4}))?$/i
  ...
  const year = yearStr ? parseInt(yearStr, 10) : getTargetYear(month, context)
  const saturdayDay = getFirstSaturdayOfMonth(year, month)
...
export function normalizeWindow(inputText, context = {}) {
  if (!inputText || typeof inputText !== 'string') {
    return {
      error: 'Please enter a date range. Examples: "Feb 7-9", "early March", "first weekend of April"'
    }
  }
  ...
  if (!result) {
    return {
      error: 'Could not understand the date format. Try: "Feb 7-9", "early March", or "first weekend of April"'
    }
  }

  // Validate window length
  const windowDays = daysBetween(result.startISO, result.endISO)
  if (windowDays > WINDOW_CONFIG.MAX_WINDOW_DAYS) {
    return {
