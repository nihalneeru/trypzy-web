...
export async function buildTripCardData(trip, userId, userRoleInCircle, db, options = {}) {
  ...
  // Get user's idea count for locked trips (for "Waiting on you" check)
  let userIdeaCount = null
  const tripStatus = trip.status || (trip.type === 'hosted' ? 'locked' : 'proposed')
  if (tripStatus === 'locked') {
    const userIdeas = await db.collection('itinerary_ideas')
      .find({ tripId: trip.id, authorUserId: userId })
      .toArray()
    userIdeaCount = userIdeas.length
  }
