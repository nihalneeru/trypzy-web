...
export async function buildTripCardData(trip, userId, userRoleInCircle, db, options = {}) {
  ...
  const userParticipantStatus = userTripParticipant?.status || 'active'
  let isCurrentUserTraveler
  if (trip.type === 'hosted') {
    // Hosted trips: must have explicit active participant record
    isCurrentUserTraveler = !!userTripParticipant && userParticipantStatus === 'active'
  } else {
    // Collaborative trips: circle members are travelers unless left/removed
    isCurrentUserTraveler = !userTripParticipant || userParticipantStatus === 'active'
  }
