    ...
    if (trip.status === 'proposed' || trip.status === 'scheduling') {
      if (trip.schedulingMode === 'top3_heatmap') {
        ...
        if (!userDatePicks || userDatePicks.length === 0) {
          ...
          // User has picked - check if everyone has responded
          const pickProgress = trip.pickProgress
          if (pickProgress && pickProgress.respondedCount >= pickProgress.totalCount && pickProgress.totalCount > 0) {
            if (isTripLeader) {
              // Leader: ready to lock
              actions.push({
                type: 'scheduling_required',
                priority: 1,
                label: 'Ready to lock dates',
                href: `/trips/${trip.id}`,
                timestamp: trip.updatedAt || trip.createdAt
              })
            } else {
              // Non-leader: waiting on leader
              actions.push({
                type: 'scheduling_required',
                priority: 1,
                label: 'Waiting on leader to lock dates',
                href: `/trips/${trip.id}`,
                timestamp: trip.updatedAt || trip.createdAt
              })
            }
