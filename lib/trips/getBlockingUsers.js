...
export function getBlockingUsers(trip, user) {
  ...
  const isTripLeader = createdById === user.id
  ...
  if (tripStatus === 'proposed' || tripStatus === 'scheduling') {
    ...
    if (!pickProgress || pickProgress.totalCount === 0) {
      return {
        blockers: [],
        reasonCode: 'initial_scheduling',
        message: isTripLeader
          ? 'Kick things off by suggesting a few time windows.'
          : 'Share a couple of time windows so everyone can weigh in.'
      }
    }

    if (pickProgress.respondedCount >= pickProgress.totalCount) {
      // Everyone has responded - check if leader needs to lock
      if (isTripLeader) {
        return {
          blockers: [],
          reasonCode: 'leader_lock',
          message: 'Dates look aligned. Lock them when you are ready.'
        }
      }
    ...
    if (blockingParticipants.length === 0) {
      ...
      return {
        ...
        message: 'Your turn to pick dates'
      }
    }
