...
export function getBlockingUsers(trip, user) {
  ...
  if (tripStatus === 'proposed' || tripStatus === 'scheduling') {
    ...
    if (!pickProgress || pickProgress.respondedCount >= pickProgress.totalCount) {
      // Everyone has responded - non-leaders are waiting for leader to lock
      if (!isTripLeader) {
        return {
          blockers: [],
      ...
      return null // Leader can lock (no blockers to show)
    }
  ...
  if (tripStatus === 'voting') {
    ...
    if (blockingParticipants.length === 0) {
      // Everyone has voted - non-leaders are waiting for leader to lock
      if (!isTripLeader) {
        return {
          blockers: [],
          reasonCode: 'leader_lock',
