...
export async function getDashboardData(userId) {
  ...
  // Get user's circle memberships (exclude left circles)
  ...
  // Get member counts and preview avatars for each circle
  const allCircleMemberships = await db.collection('memberships')
    .find({ circleId: { $in: circleIds }, status: { $ne: 'left' } })
    .toArray()

  const circleMemberUserIds = [...new Set(allCircleMemberships.map(m => m.userId))]
  const memberUsers = circleMemberUserIds.length > 0
    ? await db.collection('users')
        .find({ id: { $in: circleMemberUserIds } })
        .project({ id: 1, name: 1, image: 1, _id: 0 })
        .toArray()
    : []
  const memberUserMap = new Map(memberUsers.map(u => [u.id, u]))

  // Build per-circle memberCount + memberPreview
  const circleMemberInfo = new Map()
  for (const cId of circleIds) {
    const mems = allCircleMemberships.filter(m => m.circleId === cId)
    const preview = mems.slice(0, 4).map(m => {
      const u = memberUserMap.get(m.userId)
      return u ? { name: u.name, image: u.image || null } : null
    }).filter(Boolean)
    circleMemberInfo.set(cId, { memberCount: mems.length, memberPreview: preview })
  }
  ...
  const { filterTripsByActiveTravelerPrivacy } = await import('../trips/canViewerSeeTrip.js')
  ...
  for (const circle of circles) {
    ...
    const memInfo = circleMemberInfo.get(circle.id) || { memberCount: 0, memberPreview: [] }
    circleDataMap.set(circle.id, {
      id: circle.id,
      name: circle.name,
      role: membership.role,
      memberCount: memInfo.memberCount,
      memberPreview: memInfo.memberPreview,
      trips: []
    })
  }
  ...
  // Sort trips within each circle (separating active and cancelled)
  for (const circleData of circleDataMap.values()) {
    const sorted = sortTrips(circleData.trips)
    circleData.trips = sorted.active
    circleData.cancelledTrips = sorted.cancelled
  }
  
  // Generate global notifications from pending actions and join requests
  const globalNotifications = []
  
  // Get all trips where user is the leader (for join request notifications)
  const leaderTripIds = visibleTrips
    .filter(t => t.createdBy === userId)
    .map(t => t.id)
  
  // Fetch pending join requests for leader trips
  const joinRequests = leaderTripIds.length > 0
    ? await db.collection('trip_join_requests')
        .find({ 
          tripId: { $in: leaderTripIds },
          status: 'pending'
        })
        .sort({ createdAt: 1 })
        .toArray()
  ...
  // Get requester user info for join requests
  ...
  for (const request of joinRequests) {
    const trip = visibleTrips.find(t => t.id === request.tripId)
    if (!trip) continue
    ...
    const href = `/trips/${trip.id}?${returnParams.toString()}`
    ...
    globalNotifications.push({
      ...
      href,
      priority: 1, // High priority for join requests
