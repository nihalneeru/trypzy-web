 ...
 * @param actorRole - 'leader' | 'traveler'
 * @param actionType - What action triggered this (e.g., 'window_suggested', 'reaction_submitted')
 * @param tripCreatedAt - Trip creation date
 * @param joinedAt - When the user joined the trip (for computing delay)
 */
export async function maybeEmitFirstAction(
  ...
  let hoursSinceJoin: number | null = null
  if (joinedAt) {
 ...
 */
export async function getTravelerJoinDate(
  tripId: string | ObjectId,
  userId: string | ObjectId
): Promise<Date | null> {
  const db = await connectToMongo()
