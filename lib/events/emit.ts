...
import type { TripEventDocument, ActorRole } from './types'
 ...
 * @param circleId - Circle ID (denormalized for aggregation)
 * @param eventType - Namespaced event type (e.g., 'scheduling.window.suggested')
 * @param actorId - User ID or null for system events
 * @param actorRole - 'leader' | 'traveler' | 'system'
 * @param tripCreatedAt - Trip creation date (for computing tripAgeMs)
 * @param payload - Event-specific data (IDs only, no full documents)
 * @param options - Optional: idempotencyKey, precedingEventId, latencyFromPrecedingMs, sessionId
 * @returns Event ID string or null if duplicate
 */
export async function emitTripEvent(
  ...
  const event: Omit<TripEventDocument, '_id'> = {
    schemaVersion: 1,
