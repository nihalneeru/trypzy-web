...
export interface CircleCoordinationProfile {
  circleId: ObjectId
  tripCount: number
  ...
  avgFirstActionDelayHours: number | null
 ...
 */
export async function computeTripSnapshot(
  tripId: string
): Promise<TripCoordinationSnapshot | null> {
  ...
  // Determine outcome
  let outcome: 'active' | 'locked' | 'canceled' | 'abandoned' = 'active'
  ...
  // Time to outcome
  let timeToOutcomeHours: number | null = null
  ...
  const snapshot: TripCoordinationSnapshot = {
    tripId: new ObjectId(tripId),
    circleId: new ObjectId(trip.circleId),
    outcome,
    timeToOutcomeHours,
    windowsSuggested,
    cantReactionCount,
 ...
 */
export async function computeAllTripSnapshots(): Promise<number> {
  const db = await connectToMongo()
 ...
 */
export async function computeCircleProfile(
  circleId: string
): Promise<CircleCoordinationProfile | null> {
  const db = await connectToMongo()
  ...
  if (tripCount === 0) {
    ...
    const profile: CircleCoordinationProfile = {
  ...
  let avgFirstActionDelayHours: number | null = null
  if (firstActionEvents.length > 0) {
    const delays = firstActionEvents
      .map((e) => e.payload?.hoursSinceJoin as number)
 ...
 */
export async function computeAllCircleProfiles(): Promise<number> {
  const db = await connectToMongo()
...
export async function runDailyAggregation(): Promise<{
  tripSnapshots: number
  circleProfiles: number
}> {
