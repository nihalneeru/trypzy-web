...
function LoginPageContent() {
  ...
  useEffect(() => {
    setIsOAuthReturn(!!sessionStorage.getItem('login_oauth_pending'))
    try {
  ...
  useEffect(() => {
    ...
    const oauthPending = sessionStorage.getItem('login_oauth_pending')
    ...
    if (status === 'authenticated' && session?.accessToken) {
      ...
      if (oauthPending) {
        sessionStorage.removeItem('login_oauth_pending')
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      sessionStorage.setItem('login_oauth_pending', '1')
