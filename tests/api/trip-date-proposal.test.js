/**
 * Integration tests for trip date proposal flow (Option A)
 */
...
describe('Trip Date Proposal Flow', () => {
  ...
  it('allows collaborative trip creation without dates', async () => {
    ...
    expect(body.proposedStart).toBeNull()
    expect(body.proposedEnd).toBeNull()
  ...
  it('locks hosted trip dates at creation when dates are provided', async () => {
    ...
    expect(body.lockedStart).toBe('2025-06-10')
    expect(body.lockedEnd).toBe('2025-06-12')
    expect(body.proposedStart).toBeNull()
  ...
  it('clears reactions when leader proposes new dates', async () => {
    ...
    await db.collection('trips').insertOne({
      ...
      status: 'proposed',
      createdBy: ownerId,
      proposedStart: '2025-05-01',
      proposedEnd: '2025-05-03',
    ...
    const request = new NextRequest(`http://localhost:3000/api/trips/${tripId}/propose-dates`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    expect(updatedTrip.proposedStart).toBe('2025-06-01')
    expect(updatedTrip.dateProposalReactions).toHaveLength(0)
  ...
  it('upserts date reactions and blocks when locked', async () => {
    ...
    await db.collection('trips').insertOne({
      ...
      status: 'proposed',
      createdBy: ownerId,
      proposedStart: '2025-07-01',
      proposedEnd: '2025-07-03',
      dateProposalReactions: [],
      createdAt: new Date().toISOString()
    })

    const memberToken = createToken(memberId)
    const firstRequest = new NextRequest(`http://localhost:3000/api/trips/${tripId}/date-reaction`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const firstResponse = await POST(firstRequest, { params: { path: ['trips', tripId, 'date-reaction'] } })
    expect(firstResponse.status).toBe(200)

    let updatedTrip = await db.collection('trips').findOne({ id: tripId })
    expect(updatedTrip.dateProposalReactions).toHaveLength(1)
    expect(updatedTrip.dateProposalReactions[0].reactionType).toBe('WORKS')

    const secondRequest = new NextRequest(`http://localhost:3000/api/trips/${tripId}/date-reaction`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${memberToken}`
    ...
    const secondResponse = await POST(secondRequest, { params: { path: ['trips', tripId, 'date-reaction'] } })
    expect(secondResponse.status).toBe(200)

    updatedTrip = await db.collection('trips').findOne({ id: tripId })
    expect(updatedTrip.dateProposalReactions).toHaveLength(1)
    expect(updatedTrip.dateProposalReactions[0].reactionType).toBe('CANT')

    await db.collection('trips').updateOne(
      { id: tripId },
      { $set: { datesLocked: true, lockedStart: '2025-07-01', lockedEnd: '2025-07-03' } }
    )

    const lockedRequest = new NextRequest(`http://localhost:3000/api/trips/${tripId}/date-reaction`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${memberToken}`
    ...
    const lockedResponse = await POST(lockedRequest, { params: { path: ['trips', tripId, 'date-reaction'] } })
  ...
  it('locks dates only when approval threshold is met', async () => {
    ...
    await db.collection('trips').insertOne({
      ...
      proposedEnd: '2025-08-03',
      dateProposalReactions: [],
      createdAt: new Date().toISOString()
    })

    const ownerToken = createToken(ownerId)
    const lockRequest = new NextRequest(`http://localhost:3000/api/trips/${tripId}/lock-dates`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${ownerToken}`
      }
    })
    const lockResponse = await POST(lockRequest, { params: { path: ['trips', tripId, 'lock-dates'] } })
    ...
    const lockRequestReady = new NextRequest(`http://localhost:3000/api/trips/${tripId}/lock-dates`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${ownerToken}`
      }
    })
    const lockResponseReady = await POST(lockRequestReady, { params: { path: ['trips', tripId, 'lock-dates'] } })
    expect(lockResponseReady.status).toBe(200)

    const updatedTrip = await db.collection('trips').findOne({ id: tripId })
    expect(updatedTrip.datesLocked).toBe(true)
    expect(updatedTrip.lockedStart).toBe('2025-08-01')
    expect(updatedTrip.proposedStart).toBeNull()
